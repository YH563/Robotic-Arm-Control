...
在正向运动学中，我们希望通过关节的角度确定末端执行器的位姿(位置和姿态)，很显然，这个映射关系是唯一的。但是在一些场景下，我们希望得到在已知末端位姿的情况下，求解可能的关节角度，同样显然的是，这个问题并不是一定唯一的，可能存在多组解达到我们期望的位姿，所以逆运动学是复杂且难以求解的，同时也并不存在十分普适的求解方法，在此我们主要围绕几种主流方式进行讨论，在分类上，我们将其分为了解析法与数值法两种。
...
接下来就这两种情况进行分析。首先为三个关节轴相交于一点的情况，
