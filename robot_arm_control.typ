...
因此有，*力旋量变换公式*为，
$ cal(F)_b = ["Ad"_T_(a b)]^T cal(F)_a $
...
已知拉氏量可以表述为如下形式，
$ cal(L)(q, dot(q)) = cal(K)(q, dot(q)) - cal(P)(q) $
...
在这一节中，我们将讲述计算效率更高，物理直观性更强的牛顿-欧拉公式，从分离的线速度和角速度形式的公式，到统一的力旋量的表达，并构建力旋量与运动旋量之间的关系。
...
$ cal(F)_b = MM_s dot(cal(V))_b - ["ad"_(cal(V)_b^and)]^T MM_s cal(V)_b $
...
在一些情况下，我们期望根据当前关节状态求解每个关节的力矩，因为无法全部实时准确地得到关节速度、加速度以及力矩等物理量，所以我们需要根据关节状态对关节力矩进行计算，这样便于我们对关节进行调整，以保证最佳的运动状态。

=== 数学原理

考虑一个自由度为 $n$ 的机械臂，设{0}为基坐标系，每个连杆的坐标系框架 ${i}$ 都固定在其质心位置。现在考虑在*零时刻*，基坐标系到每个连杆的固定坐标系框架的变换矩阵定义为 $M_i = M_(0,i)$ 那么可以得到零时刻 $M_(i-1, i),M_(i,i-1)$ 的表达式分别为，
$ M_(i-1,i) = M_(i-1)^(-1) M_i\
M_(i,i-1) = M_i^(-1) M_(i-1) $

接下来考虑，在坐标系 ${i-1}$ 视角下 ${i}$ 的相对位姿，设在框架 ${i}$ 的下的旋量轴为 $cal(A)_i$，则根据旋量变换公式知道，
$ cal(A)_i^and = "Ad"_(M_i^(-1))cal(S)_i^and $

根据物理先验我们知道，两个坐标系之间的相对变换只依赖于连接这两个坐标系的关节，即 $theta_i$，那么也就是，我们可以将其他坐标系全部固定，只留下 ${i}$，那么就有，
$ T_i = exp(cal(S)_i^and theta_i) M_i = M_i exp(cal(A)_i^and theta_i) \
T_(i-1) = M_(i-1) $

因此，我们可以得到相对变换为，
$ T_(i-1, i) = T_(i-1)^(-1) T_i = M_(i-1,i)exp(cal(A)_i^and theta_i) $

设在 ${i}$ 下的空间惯性矩阵为 $MM_i$，运动旋量为 $cal(V)_i=(omega_i^T, v_i^T)^T$，力旋量为 $cal(F)_i = (m_i^T, f_i^T)^T$。

同时由在 ${i}$ 下的运动旋量的计算公式为，
$ cal(V)_i = underbrace(cal(A)_i dot(theta)_i, "关节相对运动") + underbrace(["Ad"_(T_(i, i-1))]cal(V)_(i-1), "基础传递运动") $

前一项是描述了关节自身的相对运动，也就是前一个关节的运动要加上相对运动才能继续向后传递。而后一项是基础传递运动，需要将前一项的运动转变为坐标系 ${i}$ 下的运动才行。

进而得到加速度的计算公式为，
$ dot(cal(V))_i = cal(A)_i dot.double(theta)_i + ["Ad"_(T_(i, i-1))]dot(cal(V))_(i-1) + "d"/("d"t)(["Ad"_(T_(i, i-1))])cal(V)_(i-1) $

根据前面推导牛顿欧拉公式中关于李代数伴随表示的结论，我们知道，
$ "d"/("dt")("Ad"_T) = "Ad"_T circle.small "ad"_(T^(-1)dot(T)) $

并且根据运动旋量计算公式，我们有，
$ cal(V)_(i-1)^and = "Ad"_(T_(i,i-1)^(-1)) (cal(V)_i^and - cal(A)_i^and dot(theta)_i) $

根据李群伴随表示与李代数伴随表示的定义，可以得到，
$ "d"/("d"t)("Ad"_(T_(i, i-1)))cal(V)_(i-1)^and = "ad"_(dot(T)_(i, i-1)T_(i, i-1)^(-1)) (cal(V)_i^and - cal(A)_i^and dot(theta)_i) $

同时根据 $T_(i, i-1)$ 的定义可以得到，
$ dot(T)_(i, i-1)T_(i, i-1)^(-1) &= - cal(A)_i^and dot(theta)_i exp(-cal(A)_i theta_i)M_(i, i-1)M_(i, i-1)^(-1)exp(cal(A)_i theta_i)\
&= - cal(A)_i^and dot(theta)_i $

带入上式可以得到，
$ "ad"_(dot(T)_(i, i-1)T_(i, i-1)^(-1)) (cal(V)_i^and - cal(A)_i^and dot(theta)_i) &= "ad"_(- cal(A)_i^and dot(theta)_i)(cal(V)_i^and - cal(A)_i^and dot(theta)_i)\
&= -[cal(A)_i^and dot(theta), cal(V)_i^and - cal(A)_i^and dot(theta)_i]\
&= [cal(V)_i^and, cal(A)_i^and dot(theta)]\
&= "ad"_(cal(V)_i^and)(cal(A)_i dot(theta)_i) $
...
#v(0.5em)
在机器人运动过程中，机器人控制会对目标位置进行追踪，这种机器人位置*随时间变化的函数*称为轨迹。比如需要机器人从一个位置移动到另一个位置，我们就需要设计相应的轨迹，使其能够在满足关节速度约束、加速度约束的情况下，实现机器人平滑的移动。

对于轨迹，我们会使用复合函数来表述 $theta (s(t))$，这是一个嵌套了缩放参数的函数，其中 $s in [0, 1]$，有 $theta(0)$ 在起点位置，$theta(1)$ 在终点位置，这里的 $theta$ 可以代表姿态，也可以代表关节角度，具体以真实情景为准。

同时为了保证平滑性，我们一般要求其二阶可导。

== 点对点轨迹

在这一节中，我们仅仅关注起点与终点的位置，而并不关心中间路径。

=== 任务空间轨迹生成

#figure(
  image("figure/fig13.png", width: 60%),
  caption: [线性插值]
)

先考虑一种情况，如果在空间中有起点 $(x_0,y_0)$，终点 $(x_1,y_1)$，那么我们如何计算起点和终点之间的值呢？我们可以想到，建立经过这两个点的直线方程，这样就可以计算中间点了。那么我们用一个缩放参数 $s in [0, 1]$ 来表示，那么中间点就可以写作 $s$ 的函数，
$ mat(x(s);y(s)) = mat(x_0 + s(x_1 -x_0);y_0 + s(y_1 - y_0)) $

满足 $s=0$ 的时候位于起点，$s=1$ 的时候位于终点，这就是*线性插值*算法。而对于机器人来讲，情况要略显复杂，我们需要根据机器人的起始位姿和目标位姿来进行插值，而 $S E(3)$ 群是流形空间，并非一般的线性空间，所以如果直接暴力使用线性插值，很容易导致中间插值得到的位姿逃出 $S E(3)$ 群之外。

不过我们依然可以对 $S E(3)$ 进行插值计算，我们知道，李代数是线性空间，所以可以在得到相对变换后，将其映射回 $frak(s e)(3)$ 空间后进行插值计算，然后再映射回李群空间，即可以保证其插值结果约束在李群空间下。

在基坐标系{s}下，设机械臂末端的初始位姿为 $T_0$，设其目标位姿为 $T_1$，设相对变换为 $Delta T = T_(0->1)$，则可以得到相对变换满足，
$ T_0 T_(0->1) = T_1 $
...
在李代数空间进行插值，并映射回李群空间，从而得到了插值结果为，
$ T(s) = T_0 exp(log(T_0^(-1) T_1) s) $

这样我们便得到了 *$S E(3)$ 插值算法*，这种插值算法十分平滑，且满足群约束，但是计算复杂度偏高，在实际使用的时候往往会进行近似处理。

而且，这种轨迹生成是直接在任务空间进行的，它只保证了路径的光滑性，对于关节速度、加速度等的一系列限制与要求并未考虑。所以在一些时候，我们会考虑在关节空间进行插值计算。

=== 关节空间轨迹生成

在已知初始位姿和目标位姿的情况下，我们可以根据逆运动学去计算对应的关节旋转角度，那么我们可以直接在关节空间进行线性插值，然后再利用前向运动学计算末端运动的情况，不过这种计算方式，会导致末端路径不可预测，这对于不关注末端路径的运动是适用的，但是对于要求机器人沿特定路径运动的任务上，就不太适用了。

考虑在已知关节旋转初始角度 $theta_0$ 和关节旋转目标角度 $theta_1$ 的情况下，我们可以得到关节角速度，关节角加速度的表达式为，
$ omega = dot(theta) = dot(s)(theta_1 - theta_0)\
alpha = dot.double(theta) = dot.double(s)(theta_1 - theta_0) $

那么我们接下来的任务，便是插值得到 $s$ 关于时间 $t$ 的表达式，并且保证整个运动过程尽可能平滑，不会出现速度、加速度突变的情况。主要介绍三种插值算法。

#v(0.5em)
#line(length: 100%)
#v(0.5em)

- *多项式算法*：我们主要介绍3阶多项式插值算法。

#figure(
  image("figure/fig14.png", width: 90%),
  caption: [三阶多项式插值]
)

在区间 $t in [0,T]$ 之间，设其表达式为，
$ s(t) = a_0 + a_1 t + a_2 t^2 + a_3 t^3 $

存在约束条件为，
$ cases(s(0) = 0,dot(s)(0) = 0, s(T) = 1, dot(s)(T) = 0) $

也就是初始速度与末端速度均为0，还要保证初始位置和末端位置分别为0和1。根据这些条件，我们可以得到四个未知参数的值为，
$ a_0 =0,quad a_1=0, quad a_2 = 3/T^2, quad a_3=-2/T^3 $

进而可以得到，在三阶多项式插值结果下，所需最大的关节角速度和角加速度分别为，
$ omega_max = abs(3/(2T) (theta_1 - theta_0)) $
$ alpha_max = abs(6/T^2 (theta_1 - theta_0)) $

基于上述结果，我们可以根据电机负载能力，灵活地选取运动时间，以保证电机能够提供足够的角速度和角加速度。

- *梯形算法*：

#figure(
  image("figure/fig15.png", width: 85%),
  caption: [梯形算法]
)

所谓“梯形算法”，即*速度-时间*图像是一个梯形，通过*恒加速$->$恒速$->$恒减速*三个阶段实现平滑运动，这里考虑了加速度，要比纯匀速运动要更加平滑，但是并未考虑加加速度(jerk)的存在，所以对电机依然存在不小的损耗，不过因为其低计算复杂度，可以应用于对平滑性要求不高但实时性要求高的机器人系统中。

由上图，我们可以列出加速、匀速、减速三个阶段的表达式。
$ "for"quad 0<=t<=v/a, quad cases(dot.double(s(t))&=a,dot(s(t))&=a t,s(t)&=1/2 a t^2) $
$ "for"quad v/a<t<=T-v/a, quad cases(dot.double(s(t))&=0,dot(s(t))&=v,s(t)&=v t - v^2/(2a)) $
$ "for"quad T-v/a<t<=T, quad cases(dot.double(s(t))&=-a,dot(s(t))&=a(T-t),s(t)&=(2 a v T-2v^2-a^2(t-T)^2)/(2a)) $

且有 $s(T)=1$，解得，
$ T = (a+v^2)/(v a) $

当总位移过小时，梯形算法可能会退化为三角形，不存在匀速段。

而在一些高精度场景下，进一步对梯形算法进行了改进，提出了*S型曲线*，其由七个阶段构成，如下图所示。
#figure(
  image("figure/fig16.png", width: 80%),
  caption: [S型曲线]
)

S型曲线的规划比梯形复杂得多，因为它有速度、加速度、加加速度三重约束。因此其极其平滑，加加速度恒定且有限，极大地减少了机械振动和冲击，提高了运动精度和部件寿命。但是计算非常复杂，需要处理多种情况。而在现代高性能机器人中，S型曲线已成为标准。
